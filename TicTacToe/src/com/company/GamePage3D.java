...
public class GamePage3D  extends JLayeredPane implements Runnable{

    private ExitButton exitButton;
    private JPanel panelCenter, panelLeft;
    private GameBufor gameBufor= new GameBufor();
    private GameTafel3D gameTafel3D;
    private GameBoard3D gameBoard3D;
    private JLabel timeLabel, player1Label, player2Label, vsLabel;
...
//                                                                          panelUp set
        ...
        this.add(panelBackground, JLayeredPane.MODAL_LAYER);
    ...
    public void ExitGame3D(){
        gameBoard3D= new GameBoard3D();
        ...
        panelCenter.remove(gameTafel3D);
    ...
    public void run() {
        ...
        gameTafel3D=new GameTafel3D();
        this.add(gameTafel3D, JLayeredPane.POPUP_LAYER);
        SwingUtilities.updateComponentTreeUI(this);
        gameTafel3D.setVisible(true);
        Thread gameThread = new Thread(gameTafel3D);
        ...
        float gameTime = (endGameTime - startGameTime) / 1000F; System.out.println("Time: " + gameTime + " seconds");
...
//            infoPanel.add(vsLabel);
        ...
        if(gameTafel3D.getWhoWin().equals("x")){
        ...
        else if(gameTafel3D.getWhoWin().equals("o")){
        ...
        if (!gameBufor.isGuestPlay()) {
            if (gameTafel3D.getWhoWin().equals("o")) {
            ...
            } else if (gameTafel3D.getWhoWin().equals("x")) {
                if (!gameBufor.isBotPlayer()) {
                    gameBufor.getPlayer1().setNumOfPlays(gameBufor.getPlayer1().getNumOfPlays() + 1);
            ...
            } else if (gameTafel3D.getWhoWin().equals("-")) {
                if (!gameBufor.isBotPlayer()) {
                    gameBufor.getPlayer1().setNumOfPlays(gameBufor.getPlayer1().getNumOfPlays() + 1);
        ...
        if (gameBufor.isPlayTournament()){
            ...
            if(gameBufor.getTournament().getGameRound()<2){
                ...
                while(!nextRoundButton.isPlayNextRound()){
                ...
                infoPanel.remove(player1Label);
                infoPanel.remove(player2Label);
                infoPanel.removeAll();
                gameTafel3D.removeAll();
